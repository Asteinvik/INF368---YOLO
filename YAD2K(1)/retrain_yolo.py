...
argparser.add_argument(
    ...
    default=os.path.join('model_data', 'fish_classes.txt'))

# Default anchor boxes
YOLO_ANCHORS = np.array((( 78.83591005, 142.26317639),( 31.0040678,   31.92474576),
...
def create_model(anchors, class_names, load_pretrained=True, freeze_body=True):
    ...
    detectors_mask_shape = (13, 13, 4, 1)
    matching_boxes_shape = (13, 13, 4, 5)
...
def draw(model_body, class_names, anchors, image_data, image_set='all',
    ...
    for i in range(len(image_data)):
        ...
        image_with_boxes, stats = draw_boxes(image_data[i][0], out_boxes, out_classes,
        ...
        if save_all or (len(out_boxes) > 0):
            ...
            with open(os.path.join(out_path,str(i)+'.txt'),'w') as f:
                f.write(stats)
...
def save_loss(history,stage):
    ...
    plt.savefig(stage+'_'+'loss_plot'+'.png')
    #plt.show()
    plt.close()
