...
classSizes = {"Mackerel": 35/35, "BlueWhiting": 27/35, "Herring": 33/35, "krill": 3/35, "Benthosema": 9/35}
...
def mkimage(filename, objs, names, bgs, maxobjs, output_dir="images_out",single=False):
    ...
    for c in range(0, num_obj):
        ...
        obj  = obj.resize([int(classSizes[names[cls]]*s*scale_list[c]*300/norm) for s in obj.size], Image.ANTIALIAS)
        ...
        #rescale coordinates
        posx1 = int(posx * 416/imx)
        posy1 = int(posy * 416/imy)
        posx2 = int((posx+ obj.size[0]) * 416/imx)
        posy2 = int((posy+ obj.size[1]) * 416/imy)

        log.append('{} {} {} {} {}\n'.format(names[cls], posx1, posy1, posx2 , posy2))
        boxes.append(np.array([cls,posx1, posy1, posx2 , posy2]))
        
    
    im = im.resize((416, 416), Image.BICUBIC)
    #
    #im.save(os.path.join(output_dir,filename+'.png'))
    #with open(os.path.join(output_dir,filename+'.txt'),'w') as f:
    #    for l in log: f.write(l)

    return np.asarray(boxes),np.asarray(im)
